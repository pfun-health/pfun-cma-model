...
logger = setup_logging(debug_mode=get_settings().debug)
...
class PFunDataPath:
    ...
    def __post_init__(self):
        try:
            ...
            logger.debug(
                "(failed to calculate SHA256) File not found: %s", self.raw_path
            )
        else:
            ...
            assert (
                self.sha256_digest is None
                or self.sha256_digest == _calculated_sha256_digest
            ), (
    ...
    def calc_sha256(cls, raw_path: os.PathLike | str) -> str:
        ...
        if not os.path.isfile(raw_path):
            raise FileNotFoundError(
                f"(failed to calculate SHA256) File not found: {raw_path}"
            )
        sha256_hash = sha256()
...
class PFunDataPaths:
    ...
    _pfun_data_dirpath: os.PathLike = Path(
        os.path.join(os.path.abspath(pph.get_lib_path("pfun_common")), "data")
    )
    ...
    def _sample_data_fpath(self) -> Path:
        return PFunDataPath(
            raw_path=os.path.abspath(
                os.path.join(str(self._pfun_data_dirpath), "valid_data.csv")
            ),
    ...
    def remove_sample_data(self) -> None:
        ...
        if os.path.exists(self._sample_data_fpath):
            ...
            logger.warning(
                "(attempted to remove sample data) Sample data file %s does not exist."
                + " No action taken.",
    ...
    def download_sample_data(self, overwrite: bool = False) -> None:
        ...
        if os.path.exists(self._sample_data_fpath) and not overwrite:
            logger.info(
                "Sample data already exists at %s. Skipping download.",
                self._sample_data_fpath,
            )
            return
    ...
    def download_kaggle_brist1d(self, overwrite: bool = False) -> None:
        ...
        target_parquet_path = os.path.join(
            self._pfun_data_dirpath, "brist1d_train.parquet"
        )
        if os.path.exists(target_parquet_path) and not overwrite:
            logger.info(
                "Brist1D data already exists at %s. Skipping download.",
                target_parquet_path,
            )
            return
        ...
        try:
            ...
            raise ImportError(
                "Kaggle API client not found. Please install the `kaggle` python package."
            )
        ...
        api.competition_download_file(
            "brist1d", "train.csv", path=str(self._pfun_data_dirpath)
        )
        ...
        if not os.path.exists(csv_path):
            ...
            raise FileNotFoundError(
                f"Expected to find downloaded train.csv at {csv_path}"
            )
