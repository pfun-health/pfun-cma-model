...
from pfun_cma_model.runtime.chalicelib.engine.data_utils import diff_tod_hours
from pfun_cma_model.runtime.chalicelib.engine.calc import normalize
from pfun_cma_model.runtime.chalicelib.engine.cma_plot import CMAPlotConfig
...
from typing import AnyStr, Callable, Optional, Literal, Container, Tuple, Any, Dict, List
...
class ModelResult(BaseModel):
    ...
    @validator("time", "formatted_data", "soln", "popt", "stats", pre=True, allow_reuse=True)
    def load_json(cls, v):
        if isinstance(v, str):
            return json.loads(v)
        return v
    ...
    def fixup_time(cls, value, values):
        ...
        try:
            ...
            logging.error(
                "failed to fix time vector in ModelResult!", exc_info=1)
        finally:
            return time
    ...
    def convert2dataframe(cls, v):
        ...
        if isinstance(v, pd.DataFrame):
            ...
            raise TypeError(
                f"incorrect type for {v} (ModelResult in models.py)")
        df = verify_dtypes(df)
        return df
    ...
    def append_time_column(cls, value, values):
        ...
        if "time" in values:
            ...
            value = pd.merge_asof(value, time, on="t", tolerance=0.1, )
        if is_dict:
    ...
    def ensureFloatIndex(cls, v, values):
        ...
        g_raw = pd.merge_asof(soln.sort_index().reset_index(), data, on="t")[
            ['G_x', 'G_y']].mean(axis=1).to_frame().set_index(soln.index)
        soln['g_raw'] = g_raw
        return soln
...
class ModelResultSafe(ModelResult, BaseModel):
    ...
    def ensure_sql_compatible_time(cls, vtime):
        ...
        vtime = vtime.apply(lambda d: d.isoformat() if hasattr(
            d, 'isoformat') else d).astype(str)
    ...
    def append_time_column(cls, v, values):
        ...
        if isinstance(v, pd.DataFrame) and "time" in v.columns:
            v["time"] = v['time'].apply(
                lambda t: pd.Timestamp(t).isoformat()).astype(str)
        return v
...
def _create_cmi_membermap():
    membermap = {}
    for qn, qt, qc, qr in zip(QualityNames.__args__, QualityTitles.__args__, QualityColors.__args__, QualityRanges.__args__):
...
class EndocrineSignal(Enum):
    ...
    breakfast = EndocrineSignalNS(column="g_0", label="Glucose (Breakfast)", color=mplcolors.get("purple"),
                                  tmin=7, tmax=8)
    lunch = EndocrineSignalNS(column="g_1", label="Glucose (Lunch)", color=mplcolors.get("purple"),
                              tmin=12, tmax=13)
    dinner = EndocrineSignalNS(column="g_2", label="Glucose (Dinner)", color=mplcolors.get("purple"),
                               tmin=17, tmax=18)
    ...
    def calc_hour_dist(self, v0: float | pd.Timedelta, v1: float | pd.Timedelta) -> float:
        t0, t1 = self.as_hours(v0).components.hours, self.as_hours(
            v1).components.hours
    ...
    def calc_dist(self, tmin: float, tmax: float) -> float:
        ...
        dist_raw = np.array([self.calc_dist_tmin(
            tmin), self.calc_dist_tmax(tmax)], dtype=float).max()
...
class ChronometabolicDiffCheck(BaseModel):
    ...
    def check_morn_eve_present(cls, values):
        ...
        assert all([k in values for k in kvals]
                   ), f'missing from values:\n\t"{missing}"'
...
class CMIModel(BaseModel):
    ...
    def compute_posthoc(cls, values):
        ...
        other_checks_dist = 1.0 - \
            np.nanmean([c.nominal for c in values['chrono_checks']])
        ...
        final_dist = np.nanmean(
            [normed_cmi_dist, other_checks_dist, cont_checks_dist])
...
class StrengthWeaknessItem(BaseModel):
    ...
    def get_column_label(cls, v, values):
        ...
        try:
            assert column in cma_plot_config.plot_cols, f"column {column} not in cma_plot_config.plot_cols"
        except AssertionError:
            logging.warning(
                "validator get_column_label: column %s not in cma_plot_config.plot_columns" % column, exc_info=1)
            return v
        else:
...
class StrengthWeakness:
    def __call__(self, *args, t=None, x=None, **kwds):
        ...
        if t is not None and x is not None:
            self._x = pd.DataFrame({"t": t, "x": x}, columns=["t", "x"]).set_index("t", drop=True)
        return self
    ...
    def calc_severity(self):
        ...
        if checkflag is False:
            ...
            s = (float(output) - self.value.vmin) / \
...
class Strength(StrengthWeakness, Enum):
    consistent_meals = StrengthWeaknessItem(name="consistent_meals", title="Consistent Meals",
                                            color="success", column="is_meal", func=lambda tM: (tM[tM].diff() < 7.0).all())
    not_many_highs = StrengthWeaknessItem(
        name="not_many_highs", title="Avoiding hyperglycemia", color="success", column="G", func=lambda G: (G > 0.9).mean() < 0.25,
        goal_short=(
    ...
    not_many_lows = StrengthWeaknessItem(
        name="not_many_lows", title="Avoiding hypoglycemia", color="success", column="g_raw",
    ...
    healthy_glucose_variability = StrengthWeaknessItem(
        name="healthy_glucose_variability", title="Healthy Glucose Variability", color="success", column="G", func=lambda g: g.std() < 0.2)


class Weakness(StrengthWeakness, Enum):
    frequent_lows = StrengthWeaknessItem(
        name="frequent_lows", column="g_raw", title="Recent Glucose Lows", color="danger", func=lambda g: (g < 70.0).any(), vmin=0.0, vmax=1.0,
    ...
    frequent_highs = StrengthWeaknessItem(
        column="G", title="Your blood glucose is often too high.", color="danger", func=lambda g: (g >= 1.0).mean() >= 0.4, vmin=0.0, vmax=1.0,
    ...
    inconsistent_meals = StrengthWeaknessItem(title="Inconsistent Meals", color="danger", column="is_meal", func=lambda tM: (
        tM[tM].index.to_series().abs().diff().diff() >= 2.5).mean(), vmin=0.0, vmax=1.0,
    ...
    glucose_variability_is_too_high = StrengthWeaknessItem(
        column="G", title="Glucose Variability is Too High", color="danger", func=lambda g: g.var() > 0.35, vmin=0.0, vmax=1.0)
    cortisol_peak_early = StrengthWeaknessItem(
        ...
        func=lambda c: float(c.idxmax().iloc[0]) < float(
            EndocrineSignal.cortisol.value.tmax - 1.5),
        vmin=0.0, vmax=1.0,
    ...
    cortisol_peak_late = StrengthWeaknessItem(column="c", title="Cortisol peaks too late in the day.",
                                              color="danger", func=lambda c: float(c.idxmax().iloc[0]) > float(EndocrineSignal.cortisol.value.tmax + 1.5), vmin=0.0, vmax=1.0,
                                              tod_hour_func=lambda c: float(
                                                  c.idxmax().iloc[0]),
                                              tod_hour_goal=float(
                                                  EndocrineSignal.cortisol.value.tmax),
    ...
    melatonin_peak_early = StrengthWeaknessItem(column="m", title="Melatonin peaks too early in the day.", color="danger",
                                                ...
                                                func=lambda m: m.idxmax().iloc[0] < EndocrineSignal.melatonin.value.tmax - 2, vmin=0.0, vmax=1.0,
                                                tod_hour_func=lambda m: float(m.idxmax().iloc[0]), tod_hour_goal=float(EndocrineSignal.melatonin.value.tmax))
    melatonin_peak_late = StrengthWeaknessItem(column="m", title="Melatonin peaks too late in the day.",
                                               ...
                                               color="danger",
                                               func=lambda m: float(m.idxmax().iloc[0]) > float(EndocrineSignal.melatonin.value.tmax + 1.5), vmin=0.0, vmax=1.0,
                                               tod_hour_func=lambda m: float(m.idxmax().iloc[0]), tod_hour_goal=float(EndocrineSignal.melatonin.value.tmax))
...
def calc_strengths_or_weaknesses(which: Literal["strength", "weakness"], values: Dict):
    ...
    for item in item_type:
        ...
        wdict = w.value.dict()
        wdict.update({
            "severity": w.calc_severity(),
            "tod_hour": w.calc_tod_hour()
        })
...
def ready_data_for_plotting(model_result: ModelResult | Dict):
    ...
    data_plot = model_result.formatted_data[[
        "tod", "G", "time"]].sort_values(by='tod').dropna()
    data_ewm = data_plot.set_index('time', drop=True) \
        .sort_values(by='tod') \
        .dropna() \
        .ewm(alpha=0.5)
    try:
        data_plot['gave'] = data_ewm['G'].mean().reset_index(
            drop=True).interpolate(method='akima')
...
class RecsSummaryModel(BaseModel):
    ...
    max_weakness: Optional[StrengthWeaknessItem |
                           str] = Field(alias="weakness")
    max_strength: Optional[StrengthWeaknessItem |
                           str] = Field(alias="strength")
    ...
    def _calc_max_weakness(cls, v, values, exclude: Container | None = None):
        ...
        if exclude is not None:
            ...
            to_remove = list(
                filter(lambda wk: wk.column in excluded_cols, weaknesses))
    ...
    def calc_weakness_signal_color(cls, v, values):
        ...
        if values.get('max_weakness') is not None:
            return CMAPlotConfig().get_color(values["max_weakness"].column, rgba=True)[:-1]
    ...
    def calc_weakness_signal_color_hex(cls, v, values):
        if values.get('max_weakness') is None:
            values['max_weakness'] = cls.calc_max_weakness(v, values)
        return mpl.colors.rgb2hex(CMAPlotConfig().get_color(values["max_weakness"].column, rgba=True))
    ...
    def calc_weakness_goal_color(cls, v, values):
        if values.get("weakness_signal_color") is None:
            values['weakness_signal_color'] = cls.calc_weakness_signal_color(
                v, values)
    ...
    def calc_weakness_goal_color_hex(cls, v, values):
        if values.get('weakness_goal_color') is None:
            values['weakness_goal_color'] = cls.calc_weakness_goal_color(
                v, values)
    ...
    @classmethod
    def _plot_visual_plan(cls, value, values, exclude=None, skip_glucose=False, return_weakness=False):
        ...
        there_is_weakness = all([weakness.column is not None, weakness.column != 'is_meal'])
        ...
        tstart, tend = model_result.time.min().isoformat(timespec='hours'), \
            model_result.time.max().isoformat(timespec='hours')
        tstart = tstart.split("T")[0]
        tend = tend.split("T")[0]

        #: set wear period in axis title
        fig.suptitle(f"For CGM wear period: {tstart} to {tend}" if skip_glucose is False else f"Goal for {weakness.title}")
        ...
        if skip_glucose is False:    
            ax = data_plot.plot.scatter(
                x="tod", y="gave", s="markersize", ax=axs[0], color='k',
                label=cma_plot_config.get_label("G") + " (Data)", alpha=0.5, linestyle='', marker='o'
            )
            ax.fill_between(data_plot['tod'], data_plot['gmin'], data_plot['gmax'], color='k',
                            edgecolor='k', alpha=0.2)
            ax.set_ylabel('')

            #: plot the glucose estimate (original)
            ax = wsoln.sort_values(by="t").plot.area(
                x="t", y="G", ax=ax, color=cma_plot_config.get_color("G"),
                label=cma_plot_config.get_label("G") + " (Estimate)", alpha=0.5, linestyle='--', lw=1.0
            )
        ...
        ax1 = wsoln.sort_values(by="t").plot.area(
            x="t", y=weakness.column, ax=axs[-1], color=values.get("weakness_signal_color", "r"),
            label=cma_plot_config.get_label(weakness.column), alpha=0.5, linestyle='-', lw=2.0)

        #: plot the weakness-goal
        goal_color = 'g' # ! explicit
        goal_col = weakness.column if weakness.column in nsoln.columns else 'G'
        ax1 = nsoln[["t", goal_col]].plot.area(x="t", y=goal_col, ax=ax1, color=goal_color,
                                               label=cma_plot_config.get_label(
                                                   weakness.column) + " (Goal)",
                                               alpha=0.5, linestyle='-', lw=2.0)
        if weakness.tod_hour_goal not in [None, -1]:
            ...
            ax1.axvline(weakness.tod_hour, ymin=0.0, ymax=wsoln.loc[weakness.tod_hour, weakness.column]/ymax, label="Current",
                        color="k", linestyle="--", alpha=0.3)
            y_text = -0.35
            ax1.annotate("Now", xy=(weakness.tod_hour, -0.0), xytext=(weakness.tod_hour, y_text), xycoords="data", textcoords="data",
                         fontsize=12, color="k", arrowprops=dict(arrowstyle='->', connectionstyle='arc3', color="k"))
            close_ixs = np.flatnonzero(np.isclose(nsoln.index, weakness.tod_hour_goal, atol=0.3))
            if len(close_ixs) > 0:
                close_ix = close_ixs[0]
                ymax = nsoln.iloc[close_ix, :][weakness.column]/nmax
            else:
                logging.warn(f"(models.py:visual_plan) no matching index for goal solution was found (col={weakness.column})")
                ymax = 1.0  # ! in case none is close enough
            ax1.axvline(weakness.tod_hour_goal, ymin=0.0, ymax=ymax, label="Goal", color=goal_color, linestyle="-", alpha=1.0)
            ax1.annotate("Goal", xy=(weakness.tod_hour_goal, -0.0), xytext=(weakness.tod_hour_goal, y_text), xycoords="data", textcoords="data",
                         fontsize=12, color=goal_color, arrowprops=dict(arrowstyle='->', connectionstyle='arc3', color=goal_color))
    ...
    def calc_visual_plan(cls, value, values):
        ...
        iw = 1
        while True:
            fig, axs, weakness = cls._plot_visual_plan(value, values, exclude=weaknesses,
                                                       return_weakness=True, skip_glucose=True)
